...
import { deleteListing } from "@/lib/database"
import { createClientComponentClient } from "@supabase/auth-helpers-nextjs"

interface DeleteSelectedListingsButtonProps {
  userId: string;
  selectedListings: string[];
  onSuccess: () => void;
}

export default function DeleteSelectedListingsButton({ 
  userId, 
  selectedListings, 
  onSuccess 
}: DeleteSelectedListingsButtonProps) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      // Delete each selected listing one by one
      const results = await Promise.allSettled(
        selectedListings.map(listingId => deleteListing(listingId))
      );
      ...
      setResult({
        ...
        message: `Successfully deleted ${successCount} listings${failureCount > 0 ? `, failed to delete ${failureCount} listings` : ''}`
      })
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {isDeleting ? "Deleting..." : `Delete Selected (${selectedListings.length})`}
      </button>
      ...
      {showConfirmation && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-xl font-bold text-white mb-4">Delete Selected Listings</h3>
            <p className="text-gray-300 mb-6">
              Are you sure you want to delete {selectedListings.length} selected listings? This action cannot be undone.
            </p>
