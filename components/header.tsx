...
export default function Header() {
  ...
  return (
    <header
      className={`backdrop-blur-md bg-black/70 border-b border-white/10 shadow-lg z-50 fixed w-full top-0 left-0 transition-transform duration-500 ${
      ...
      <div className="container mx-auto px-4 py-2 flex justify-between items-center relative min-h-[56px] sm:min-h-[48px]">
        ...
        <nav className="hidden md:block">
          <ul className="flex space-x-8 items-center">
            ...
            {/* Puedes descomentar estos cuando estén listos */}
            {/* <li>
            ...
            <li>
              <Link
                href="/login"
                className="font-semibold text-base px-6 py-2 rounded-full bg-gradient-to-r from-primary-pink to-primary-purple text-white shadow-md hover:from-primary-purple hover:to-primary-pink transition-all duration-300 border-0"
              >
                Sign In
              </Link>
            </li>
        ...
        {/* Mobile nav */}
        <nav className={`fixed top-0 right-0 left-0 z-50 md:hidden transition-transform duration-500 ${menuOpen ? 'translate-y-0' : '-translate-y-full'}`}>
          <ul className="flex flex-col items-center justify-center bg-black/95 pt-24 pb-8 space-y-8 min-h-screen text-lg font-semibold">
            ...
            <li>
              <Link href="/login" onClick={() => setMenuOpen(false)} className="block w-full text-center py-3 px-10 rounded-full bg-gradient-to-r from-primary-pink to-primary-purple text-white shadow-md hover:from-primary-purple hover:to-primary-pink transition-all duration-300 border-0">
                Sign In
              </Link>
            </li>
          </ul>
