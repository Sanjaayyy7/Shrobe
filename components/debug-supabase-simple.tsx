...
export default function DebugSupabaseSimple() {
  ...
  const checkConnection = async () => {
    ...
    try {
      if (isDemoMode) {
        ...
        setStatus("Demo mode: Mocked database connection")
        setTableExists(true)
        return
      }
      
      // Just check if the signups table exists
  ...
  if (!isVisible) {
    return (
      <button 
        ...
        className="fixed bottom-4 right-4 bg-gray-800 text-white p-2 rounded-full w-8 h-8 flex items-center justify-center shadow-lg hover:bg-gray-700 focus:outline-none z-50"
        title="Show Debug Panel"
      >
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
          <path d="M12 19l9 2-9-18-9 18 9-2zm0-15v3" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none" />
        </svg>
      </button>
    )
  }
  ...
  return (
    <div className="fixed bottom-4 right-4 bg-gray-800 p-4 rounded-lg shadow-lg text-sm z-50 max-w-md">
      <div className="flex justify-between items-center mb-2">
        <h4 className="font-bold">Supabase Debug</h4>
        <button 
          onClick={toggleVisibility}
      ...
      {isDemoMode && (
        <div className="bg-yellow-800 text-yellow-200 p-2 rounded mb-2">
          ...
          <p className="text-xs">
            Using mocked database. Set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY 
            environment variables in .env.local file to connect to a real database.
          </p>
          <p className="text-xs mt-1">
            Current mock data: {mockSignups.length} signup{mockSignups.length !== 1 ? 's' : ''}
          </p>
        </div>
      )}
      ...
      {isDemoMode && (
        <div className="mt-4 border-t border-gray-700 pt-2">
          <p className="text-xs text-gray-300">
            To setup a real database:
          </p>
          <ol className="list-decimal pl-4 mt-1 space-y-1 text-xs text-gray-300">
            <li>Create a <a href="https://supabase.com" target="_blank" rel="noopener noreferrer" className="text-primary-purple hover:underline">Supabase</a> account</li>
